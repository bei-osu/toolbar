// ==UserScript==
// @name         osu! Modding TB
// @namespace    https://github.com/bei-osu/toolbar/blob/main/toolbar.js
// @version      2025.11.04
    ...
    class ComparisonMode {
        ...
        static pruneCache() {
            if (this.cache.size > this.MAX_CACHE_SIZE) {
                const toDelete = this.cacheAccessOrder.splice(0, this.cache.size - this.MAX_CACHE_SIZE);
...
`;
    ...
    class VocabularyService {
        ...
        async getDefinition(word) {
            ...
            try {
                ...
                if (response?.[0]) {
                    ...
                    VocabularyService.setCached ? VocabularyService.setCached(cacheKey, result) : this.cache.set(cacheKey, result);
                    return result;
                }
            } catch (error) {
        ...
        async getSynonyms(word) {
            return this.queueRequest(() => this.fetchDatamuse(word, 'syn', 'rel_syn'));
        }
        async getSimilar(word) {
            return this.queueRequest(() => this.fetchDatamuse(word, 'sim', 'ml', word));
        }
        async getRhymes(word) {
            return this.queueRequest(() => this.fetchDatamuse(word, 'rhyme', 'rel_rhy', null, 20));
        }
        async getFollowing(word) {
            return this.queueRequest(() => this.fetchDatamuse(word, 'follow', 'lc', null, 20));
        }
        async getAdjectives(word) {
            return this.queueRequest(() => this.fetchDatamuse(word, 'adj', 'rel_jjb', null, 20));
        }
        async getAssociations(word) {
            return this.queueRequest(() => this.fetchDatamuse(word, 'assoc', 'rel_trg', null, 20));
        }
    ...
    class OsuTB {
        ...
        handleGlobalError(event) {
            ...
            if (this.errorCount > this.maxErrors) {
                UI.showNotification('TB experiencing errors - reloading...', 'error');
        ...
        setupEventListeners() {
            ...
            if (!this.TB.dataset.draggableAttached) {
